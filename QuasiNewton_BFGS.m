...
function [xopt,fopt,niter,gnorm,dx] = QuasiNewton_BFGS(varargin)
...
while and(gnorm>=tol, and(niter <= maxiter, dx >= dxmin))
    ...
    elseif(i > 0)
        ...
        yk = gnew - gold;
        sk = x - xold;
        Hk1 = Hk + (((1+(yk'*Hk*yk)/(sk'*yk))*sk*sk')-(Hk*yk*sk')-(sk*yk'*Hk))/(sk'*yk); % BFGS
        
        pk1 = -Hk1*gnew;
        
        if(pk1'*gnew <= 0)
            [tk1,~] = fminbnd(@(tk) iterbyQuasiNewton(tk,x,pk1),0,0.11);
            xnew = x + tk1  *  pk1;
            pk = pk1;
        else
            i = 0;
